 ...
 * Modificated by Hernan Tonutti on 26/03/2019
 *
 * CAMBIOS
 * Agregada funcion verNodosPorNombre
 *
 */
public class ArmaCamino{
    ...
    //Funcion para generar un vector con las aulas por Edificio
    ...
    //Funcion para generar un vector con nodos que contengan el nombre pasado
    public Vector<Punto> verNodosPorNombre(String nombre, int modo) {
        Vector<Punto> nodos = new Vector<>();

        //Si modo es 0 se fija si contiene el nombre
        if (modo == 0) {
            for (int i = 0; i < Nodos.size(); i++) {
                if (Nodos.elementAt(i).getNombre().contains(nombre)) {
                    nodos.add(Nodos.elementAt(i));
        ...
        //Si modo es 1 se fija si empieza con el nombre
        else if (modo == 1) {
            for (int i = 0; i < Nodos.size(); i++) {
                if (Nodos.elementAt(i).getNombre().startsWith(nombre)) {
                    nodos.add(Nodos.elementAt(i));
                }
            }
        }
